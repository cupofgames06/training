...
<div data-kt-daterangepicker="true" data-kt-daterangepicker-opens="left" data-kt-daterangepicker-range="today" class="btn btn-sm btn-primary d-flex align-items-center px-4" data-kt-initialized="1">
    <!--begin::Display range-->
    ...
    <script type="module">
        document.addEventListener("DOMContentLoaded", function (event) {
            ...
            const indicators = function (start,end){
                let route = "{{ route('of.monitoring.customer.indicators') }}";

                axios.get(route, {
                    params: {
                        date_start: start.format('YYYY-MM-DD'),
                        date_end: end.format('YYYY-MM-DD'),
                        @if(Route::current()->getName() === 'of.monitoring.customer.leaner.details')
                            learner: {{ $learner->id }},
                        @else
                            company: {{ $item->id }}
                        @endif
            ...
            const ratings = function (start,end){
                let route = "{{ route('of.monitoring.customer.ratings') }}";
            ...
            const overview = function (start,end){
                let route = "{{ route('of.monitoring.customer.overview') }}";
