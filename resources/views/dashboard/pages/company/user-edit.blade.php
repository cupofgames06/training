<x-auth-layout>
    ...
    <x-form :action="route('company.users.update',[$user])" id="manager_create_form" v-on:submit="checkForm">
        @method('patch')
        @csrf
        <x-content.breadcrumb
            :breadcrumbs="[route('company.users.index')=>trans('common.manager.index.title'),'#'=>trans('common.manager.create.title')]"></x-content.breadcrumb>
        ...
        @component('components.content._sticky-footer')
            ...
            @slot('buttons')
                <button class="btn btn-outline-danger me-3"
                        data-url="{{ route('company.users.delete',['user'=>$user]) }}"
            ...
            @empty($user->password)
                ...
                <button class="btn btn-outline-danger me-3"
                        data-url="{{ route('company.users.reinvite',['user'=>$user]) }}"
